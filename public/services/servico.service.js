angular.module("app").service("ServicoService", ServicoService);
...
function ServicoService($http) {
  ...
  service.update = function(id, servico) {
    return $http.put(URL + "/" + id, servico).then(function(resp) {
      return resp.data;
    });
  };
  ...
  service.insert = function(servico) {
    return $http.post(URL, servico).then(function(resp) {
      return resp.data;
    });
  };
}
