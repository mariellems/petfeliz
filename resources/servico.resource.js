...
module.exports = function(app) {
  ...
  app.post("/servicos", function(req, resp) {
    servicoModel.create(req.body).then(
      function(data) {
  ...
  app.get("/servicos/:id", function(req, resp) {
    servicoModel.findById(req.params.id).then(
      function(data) {
        if (!data) {
  ...
  app.put("/servicos/:id", function(req, resp) {
    servicoModel.findOneAndUpdate({ _id: req.params.id }, req.body).then(
      function(data) {
        if (!data) {
          resp.status(404).send();
        } else {
  ...
  app.delete("/servicos/:id", function(req, resp) {
    servicoModel.deleteOne({ _id: req.params.id }).then(
      function() {
